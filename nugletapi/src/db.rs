...
use http:: Uri;
    ...
    pub(super) fn get_images_by_vote(conn: &Connection, votes: u32) -> Result<Vec<FlickrImage>, Box<std::error::Error>> {
...
fn from_rows(rows: &mut Rows) -> Result<Vec<FlickrImage>, Box<std::error::Error>> {
        ...
        while let Some(row) = rows.next() {
            let row = row?;
            images.push(FlickrImage::new(row.get::<usize, String>(0), row.get::<usize, String>(1).parse()?, row.get::<usize, u32>(2)));;
        }
        Ok(images)
}